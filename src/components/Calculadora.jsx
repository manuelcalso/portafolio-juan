...
const Calculator = () => {
    ...
    const calculateCompoundInterest = () => {
        ...
        for (let i = 1; i <= t * 12; i++) {
            futureValue += contribution * Math.pow((1 + rate / n), n * ((t * 12 - i + 1) / 12));
            if (i % 12 === 0) {
    ...
    return (
        <>
            ...
            <div className="max-w-lg mx-auto m-10 p-6 bg-white rounded-lg shadow-md">
                ...
                <button onClick={calculateCompoundInterest} className="w-full py-2 bg-blue-500 text-white font-bold rounded-md hover:bg-blue-600">Calcular</button>
                <section className='flex-col'>
                    ...
                    {futureValue !== null && (
                        <div className="mt-6 text-center">
                            <h3 className="text-xl font-bold text-black">Intereses Generados: ${formatNumber(totalInterest)}</h3>
                            <h3 className="text-xl font-bold text-black">Valor Futuro: ${formatNumber(futureValue)}</h3>
                        </div>
                    )}
                </section>
            </div>
